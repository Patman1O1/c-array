...
namespace array_testing {
    ...
    /* ----------------------------------------------------array_swap------------------------------------------------ */
    TEST(array_swap_suite, empty_arrays) {
        struct ::array array1 = {};
        struct ::array array2 = {};

        array_init(int, array1, 0);
        array_init(int, array2, 0);

        ::array_swap(&array1, &array2);

        EXPECT_EQ(::array_free(&array1), EXIT_SUCCESS);
        EXPECT_EQ(::array_free(&array2), EXIT_SUCCESS);
    }
